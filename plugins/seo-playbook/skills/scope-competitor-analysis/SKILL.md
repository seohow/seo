...
6. **Define the deliverable.** Specify exactly what comes out: a synthesis doc with sections for each lens, a per-competitor sub-doc, and a prioritised opportunity list. The opportunity list is the most important output — it's what feeds the [build-seo-wins-backlog](../build-seo-wins-backlog/SKILL.md) workflow.
