...
- Decided released skill artifacts are generated by CI from `v*` tags and tracked as GitHub Release assets. The generated release tree should keep a human-browsable `setup/` and `categories/<category>/` folder structure.
...
### 2026-05-06 — Release ZIP layout flattened

- Release output no longer creates one folder per skill. Setup skills publish as `setup/<skill>.zip`; category skills publish as `categories/<category>/<skill>.zip`.
- Reason: the release version folder already provides version context, and the ZIP filename is enough to identify the upload unit.
- Each ZIP still contains exactly one skill folder at the archive root.

### 2026-07-05 — Full repo review executed; conventions made self-enforcing

- Ran a full repo review (link integrity, catalogue sync, template conformance, example drift, doc-vs-reality drift). Findings and execution status live in root `TODO.md`; work was delegated per-task to Haiku/Sonnet/Opus and reviewed.
- Fixed AGENTS.md doc drift: category-slug list now names all 10 slugs; `skills/README.md` is documented as a two-level catalogue (top-level index + per-category READMEs, which are now the maintenance surface for skill changes); repo-layout tree now includes `scripts/`, `releases/`, `.github/`, release configs, and per-category skill READMEs.
- Named Field & Sun in the 6 worked examples that still used an anonymous "D2C beauty brand". Deliberate exception kept: Technical SEO's JavaScript SEO and Log File Analysis leaves use "a hypothetical brand" because their scenarios require a headless/Vercel stack Field & Sun (Shopify) can't host.
- Added a "Last reviewed" dating convention to all `09. AI SEO` pages with a quarterly re-verification note in the category README. First refresh pass due ~2026-10.
- Hardened validation: `validate-skills.mjs` now enforces required SKILL.md sections (setup skills exempt from topic-only sections by archetype); new `check-links.mjs` (repo-wide relative-link + %20 checker, templates and AGENTS.md illustrative examples allowlisted) and `check-consistency.mjs` (catalogue sync + stale-language scan). Wired into `package.json` (`repo:check`) and the release workflow before packaging. Reason: conventions were previously enforced only by agent discipline; now CI catches drift.
- Skill polish: `analyse-log-files` gained a no-log-access fallback (GSC Crawl Stats proxy, explicit Shopify limitation, stop rule against fabricating crawl data); sharpened under-triggering descriptions on `plan-link-building-campaign`, `monitor-brand-mentions`, and `design-citation-magnet-content` (now names ChatGPT/Perplexity/AI Overviews).
- Open owner decisions deferred, recorded in `TODO.md`: analytics artifact-subfolder consolidation (recommend leave as-is) and inventory-table location in AGENTS.md (recommend keep, bump date when re-verified).

### 2026-07-05 — Owner decisions closed, first AI SEO refresh, plugin deployment

- Analytics artifact subfolders: decided to leave as-is (all 7 analytics skills write to `businesses/<slug>/analytics/`; filenames prevent collisions). Revisit only if the folder gets noisy in real use.
- Structural inventory moved out of AGENTS.md into root `INVENTORY.md` — a dated, append-only ledger — so counts don't pollute agent-instruction context. AGENTS.md keeps a pointer. Append a snapshot row when categories, leaves, or skills change.
- First quarterly AI SEO refresh executed (web-verified against the July 2026 landscape). Material updates: `Claude-Web` replaced by `Claude-User` + `Claude-SearchBot` in `audit-ai-crawler-access`; `Perplexity-User` added; llms.txt reframed as unratified with no confirmed engine adoption (Google explicitly doesn't use it) in `generate-llms-txt` and the audit skill; Google AI Mode added alongside AI Overviews in the AI SEO leaves; LLM-visibility tooling updated (Ahrefs Brand Radar, Copilot); GA4 AI-referral domains refreshed (`chatgpt.com`, native AI Assistant channel). Citation-count claims left hedged as unverifiable-but-corroborated. Recurring quarterly run scheduled (1 Jan/Apr/Jul/Oct); next 2026-10-01.
- Repo is now deployable as a Claude plugin: `.claude-plugin/plugin.json` (name `seo-toolkit`, skills paths enumerate the 10 category dirs since nested auto-discovery isn't reliable) and `marketplace.json` (repo installs via `claude plugin marketplace add seohow/seo`). New `npm run plugin:package` builds `releases/seo-toolkit.plugin` (skills + playbook + templates; `businesses/` deliberately excluded — workspaces belong to the user). `check-consistency.mjs` now validates the manifest: JSON parses, skills paths exist, version matches package.json, marketplace entry present. `.plugin` artifacts gitignored.
