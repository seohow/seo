...
description: Produces the outreach operational blueprint covering domain + authentication setup (dedicated subdomain, SPF / DKIM / DMARC), sender-warming plan (4-6 week ramp for new domains), ESP / tooling choice (Lemlist / Mailshake / Postaga / Apollo / etc.), sender-identity strategy (named human, not generic), template library across channels (PR / guest / mention-recovery / broken-link / HARO follow-up; each with personalisation variables), CRM schema + tooling (Pipedrive / HubSpot / Streak / Close), follow-up cadence (Day 0 / 4 / 10 / 14), quality controls (pre-send review, response-rate monitoring, deliverability monitoring), and compliance (GDPR / CAN-SPAM / PECR — unsubscribe, identifying info, no scraping). Output is the operational manual that all tactical-channel skills consume. Use whenever the user asks to "design our outreach process," "set up outreach," "improve our pitch response rate," "set up cold outreach," "outreach CRM," "deliverability is bad — what do we do," or has tactical-channel work but no operational layer underneath. Also use as an annual cadence skill — outreach infrastructure drifts (templates get stale, sender reputation decays, CRM hygiene slips). Do not use to plan a single PR campaign (use `plan-digital-pr-campaign`), guest-post campaign (use `plan-guest-posting-campaign`), or to design the broader off-page strategy (use `plan-link-building-campaign`).
---
