...
description: Produces a content brief specifically structured to earn citation in AI Overviews, Perplexity, ChatGPT search, Claude search, and Gemini for a specific topic that has been prioritised in `plan-generative-seo-strategy`. The brief specifies the question framing, definitive answer block, TL;DR, primary-evidence requirements (original photography, testing data, surveys, primary research), comparative-table structure, source-citation list with named authoritative sources, schema spec (Article / Product / FAQPage / ImageObject as appropriate), internal-linking plan, freshness-signal requirements, and per-engine targeting notes. Use whenever the user asks to "design citation-magnet content," "write a content brief for AI citations," "brief for AI Overview citation," "GEO content brief," "AEO brief," "design content to be cited by ChatGPT / Perplexity / Claude," or after `plan-generative-seo-strategy` has identified a priority topic. The skill requires a green-lit topic from `plan-generative-seo-strategy` as input — refuses to brief a topic that hasn't been strategically vetted (citation-magnet content investment is expensive; running it without a strategic plan wastes the budget). Do not use to plan strategic investment (use `plan-generative-seo-strategy`), to audit existing pages (use `audit-ai-content-readiness`), or to produce a generic content brief without AI-citation focus (use `generate-content-brief` in Content SEO).
---
