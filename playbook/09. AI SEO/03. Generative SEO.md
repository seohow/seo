...
- **Engine-specific citation patterns.**
  - **AI Overviews (Google):** cites 3-5 sources per Overview; sources tend to be the highest-authority pages also winning featured snippets and high SERP positions. Citation is a card with link + brief excerpt. Tends to favour established publishers + well-structured brand content. As of 2026 AI Overviews appear on roughly a quarter of US searches (and ~half of informational queries), and connect directly into **Google AI Mode** — the conversational, agentic search surface that has become a default experience and cites sources in the same authority-plus-structure pattern. Treat AI Mode citation as part of the same Google workstream, not a separate game.
