...
description: Produces a geo-targeting strategy + audit covering Hreflang reinforcement (cross-references `audit-hreflang`), GSC International Targeting setting per gTLD property (deprecated for ccTLDs but useful for subdirectory / subdomain setups), server / CDN strategy (low-priority signal in 2026 but worth noting), currency display per market (USD / GBP / EUR / etc.), local content cues (phone numbers, addresses, shipping language, units, spelling — US English vs UK English), imagery audit (generic-global vs market-specific), local-backlink-strategy plan (per-market off-page priority), and per-market signal-consistency check (flagging mixed signals like US currency on UK pages). Output is the per-market geo-signal plan with prioritised remediations. Use whenever the user asks to "plan geo targeting," "set up country targeting," "UK visitors landing on US pages," "same-language different-market issues," "geo signal audit," "international targeting strategy," or operates in same-language multi-market and is investigating ranking / conversion gaps. Also use as a quarterly cadence skill — geo signals drift as new pages ship and content gets edited. Do not use to audit Hreflang specifically (use `audit-hreflang`), to decide architecture (use `decide-multi-language-architecture`), or to plan content localisation (use `plan-localisation`).
---
