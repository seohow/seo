...
description: Produces the per-market localization plan covering translation method (machine vs machine+human-review vs human-professional vs native-in-house), cultural adaptation strategy (imagery, examples, customer references, market-specific positioning), regulatory + legal compliance per market (GDPR, Cookie Directive, EU Accessibility Act, country-specific consumer protection laws like Toubon Law in France or Verbraucherschutz in Germany), brand-voice variation guide (formality / tone norms per market while preserving brand identity), currency + pricing display + payment methods, date / number formats, SEO-specific keyword research per market (research fresh — don't translate keywords literally), priority template sequence (homepage → pillar → PDPs → checkout), quality-control plan (native-speaker review), and translation-memory strategy (Smartling / Crowdin / Lokalise / Phrase). Output is the operational plan that engineering, content, and legal can execute against. Use whenever the user asks to "plan localisation," "translate the site," "expand to [market]," "localise content for [market]," "we're launching in [country]," or has decided on multi-language architecture and is moving to content adaptation. Also use as an annual cadence skill — localization tooling, regulations, and market norms shift. Do not use to audit Hreflang (use `audit-hreflang`), decide architecture (use `decide-multi-language-architecture`), or to set up geo-targeting signals beyond content (use `plan-geo-targeting`).
---
