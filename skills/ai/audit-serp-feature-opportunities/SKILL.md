...
description: Produces a per-priority-keyword feature audit covering presence, current source / holder, reachability assessment, and per-feature requirements (content shape, schema, structural changes) across featured snippets (paragraph / list / table), People Also Ask, AI Overviews, image packs, video carousels, shopping packs, knowledge panels, and FAQ rich results. Output is a prioritised feature-by-feature plan: which features to compete for per query, what's required to win each, and which features to explicitly skip (e.g. knowledge panels for non-notable brands; featured snippets dominated by Wikipedia or established publishers). Use whenever the user asks to "audit SERP features," "find feature opportunities," "featured snippet plan," "PAA strategy," "rich result audit," "win the AI Overview," "shopping pack audit," "image pack visibility," or has a priority keyword set and wants to know where the on-SERP wins are. Also use as a quarterly cadence per priority cluster — feature presence shifts as Google rolls out feature changes and competitors capture / lose snippets. Do not use to design net-new content (use `design-citation-magnet-content`), to audit prose-level patterns (use `audit-ai-content-readiness`), or to plan strategic generative-SEO investment (use `plan-generative-seo-strategy`).
---
